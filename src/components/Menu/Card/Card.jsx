...
export default function Card({food, className = "", lang = "es"}) {
  ...
  return (
    <div className={twMerge(className, `flex flex-col min-w-[200px] md:min-w-[200px] max-w-[250px] bg-white rounded-lg shadow-md ${hasImage ? "mb-3" : ""}`)}>
      {hasImage 
        ? <img src={food.image ? food.image : "/img/placeholder-image.jpg"} 
            alt={food.name} 
            className='w-full aspect-[1/0.8] object-cover rounded-lg mb-2' /> 
        : null}
